...
export const getTrackRes = async (entryUrl) => {
    const response = await Stack.getEntryByUrl({
    ...
    liveEdit && addEditableTags(response[0], "track", true);
    return response[0];
};
...
export const getAllTracks = async () => {
    ...
    liveEdit &&
        response[0].forEach((entry) => addEditableTags(entry, "track", true));
    return response;
};
...
export const getComposableHeroGallery = async (entryUrl) => {
    console.log("getComposableHeroGallery" , entryUrl);
    const response = await Stack.getEntryByUrl({
...
export const getSuperheroGalleryRes = async () => {
    console.log("getSuperheroGalleryRes");
    const response = await Stack.getEntry({
        contentTypeUid: "character",
        jsonRtePath: ["description"],
    });
